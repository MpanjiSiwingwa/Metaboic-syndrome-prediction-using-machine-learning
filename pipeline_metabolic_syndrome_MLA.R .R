...
data_file   <- here::here("MetSyn_dataset_Week_144.csv")
...
dirs <- c(results_dir, figures_dir, models_dir, tables_dir)
...
## 4.5 BASELINE CHARACTERISTICS TABLES

# Function to Generate Table 1 for a Given Dataset Split
...
generate_table1 <- function(data, set_label) {
  ...
  data %>%
    ...
    dplyr::mutate(
      # Age grouped at 40 years
      Age_group = factor(
        ifelse(Age_years < 40, "< 40 years", "\u2265 40 years"),
        levels = c("< 40 years", "\u2265 40 years")
      ),
      
      # Viral load grouped at 3.0 log10 copies/mL (1000 copies/mL)
      ViralLoad_group = cut(
        Log10_Viral_Load,
        breaks = c(-Inf, 3.0, Inf),
        labels = c(
          "< 3.0 log10 (< 1000 copies/mL)",
          "\u2265 3.0 log10 (\u2265 1000 copies/mL)"
        )
      ),
    ...
    gtsummary::tbl_summary(
      ...
      label = list(
        Age_group       ~ "Age Group",
        ViralLoad_group ~ "Viral Load Group",
        Sex             ~ "Sex",
    ...
    gtsummary::add_p(
      test = list(
        ...
        ViralLoad_group ~ "fisher.test",
        Sex             ~ "chisq.test",
        Alcohol_Use     ~ "chisq.test"
      ),
...
# Add Dataset Labels Before Combining

# Create copies and add Set identifier
train_set_labeled <- train_set %>%
  dplyr::mutate(Set = "Training Set")

test_set_labeled <- test_set %>%
...
table1_flex <- gtsummary::as_flex_table(table_combined) %>%
  flextable::add_footer_lines(
    values = c(
      ...
      "P-values were calculated using Pearson's Chi-squared test or Fisher's exact test when expected cell counts were < 5."
    )
  ) %>%
  flextable::autofit()
...
print(
  doc,
  target = here::here("tables", "Table1_Baseline_Characteristics_MetS.docx")
)
...
# Add trained xgboost model into all_models
all_models$original$xgboost <- xgb_results_list[["original"]]$model
...
evaluate_model_youden <- function(model, test_data, model_name = "model",
  ...
  tryCatch({
    ...
    if (grepl("xgboost", model_name)) {
      x_test <- model.matrix(Metabolic_Syndrome ~ ., data = test_data)[, -1]
      probs_raw <- predict(model, data.matrix(x_test))
    } else {
      probs_raw <- tryCatch(
...
all_metrics <- map_dfr(dataset_names, function(dname) {
  ...
  map_dfr(names(models), function(m) {
    ...
    probs <- tryCatch({
      if (m == "xgboost") {
        # Build design matrix for xgboost
        x_test <- model.matrix(Metabolic_Syndrome ~ ., data = datasets[[dname]]$test)[, -1]
        predict(models[[m]], data.matrix(x_test))
      } else {
        predict(models[[m]], datasets[[dname]]$test, type = "prob")[, "Yes"]
      }
    }, error = function(e) {
      cat("⚠️ Prediction failed for", m, ":", e$message, "\n")
      return(rep(NA, nrow(datasets[[dname]]$test)))
    })
...
safe_predict_probs <- function(model, newdata, model_name = "model") {
  out <- tryCatch({
    if (grepl("xgboost", model_name)) {
      x_test <- model.matrix(Metabolic_Syndrome ~ ., data = newdata)[, -1]
      return(as.numeric(predict(model, data.matrix(x_test))))
    }
    ...
    cat("⚠️ Probability extraction failed for", model_name, ":", e$message, "\n")
...
pr_data <- map_dfr(dataset_names, function(dname) {
  ...
  map_dfr(names(models), function(m) {
    if (!is.null(models[[m]])) {
      ...
      if (!is.null(probs)) {
        plot_pr_from_probs(probs, labels_bin, paste(dname, m, sep = "_"))
      }
    }
  })
})
...
figure_3a <- ggplot(pr_data, aes(x = Recall, y = Precision, color = Model)) +
  geom_line(linewidth = 1.2) +
  geom_hline(data = auc_pr_labels, aes(yintercept = Baseline, color = Model),
             linetype = "dashed", linewidth = 0.5, alpha = 0.6) +
  labs(
    title = "Figure 3a: Precision-Recall Curves for Metabolic Syndrome Prediction",
...
ggsave("Figure_3a_PR_Curves_BMC.tiff", plot = figure_4,
       width = 7.5, height = 6.5, dpi = 300, compression = "lzw", device = "tiff")
ggsave("Figure_3a_PR_Curves_BMC.png", plot = figure_4,
...
results_summary <- results_summary %>%
  mutate(AUC_CI = paste0("[", round(AUC_Lower, 3), ", ", round(AUC_Upper, 3), "]"))
...
for (i in 1:nrow(results_summary)) {
  ...
  if (!is.null(model_obj)) {
    ...
    probs      <- safe_predict_probs(model_obj, test_data, paste(dataset_name, model_type, sep = "_"))
...
# ===============================================================
# 8.6 CALIBRATION ANALYSIS FOR ALL MODELS
# ===============================================================
...
for (dataset_name in names(all_models)) {
  ...
  for (model_name in names(all_models[[dataset_name]])) {
    ...
    train_probs <- safe_predict_probs(
      model, train_data,
      paste(dataset_name, model_name, "train", sep = "_")
    )
    
    test_probs <- safe_predict_probs(
      model, test_data,
      paste(dataset_name, model_name, "test", sep = "_")
    )
    ...
    # Calibration methods
    platt_probs <- apply_platt_calibration(
      train_probs, train_labels, test_probs
    )
    
    iso_probs <- suppressWarnings(
    ...
    # Store metrics
    calibration_results[[model_id]] <- dplyr::bind_rows(
      get_cm_metrics_youden_from_probs(
        test_probs, test_labels,
        paste(model_id, "Raw")
      ),
      get_cm_metrics_youden_from_probs(
        platt_probs, test_labels,
        paste(model_id, "Platt")
      ),
      get_cm_metrics_youden_from_probs(
        iso_probs, test_labels,
        paste(model_id, "Isotonic")
      )
    )
  }
}
...
calibration_table <- dplyr::bind_rows(calibration_results)

# Create AUC confidence interval column
calibration_table <- calibration_table %>%
  dplyr::mutate(
    AUC_CI = ifelse(
      !is.na(AUC_Lower) & !is.na(AUC_Upper),
      paste0("[", round(AUC_Lower, 3), ", ", round(AUC_Upper, 3), "]"),
      NA_character_
    )
  )
...
# Format Calibration Table for Word Export

ft_calibration <- flextable::flextable(
  calibration_table %>%
    dplyr::select(
      Model,
      AUC,
    ...
    dplyr::mutate(
      dplyr::across(
        c(AUC, PR_AUC, Sensitivity, Specificity),
        ~ round(.x, 3)
      )
    )
) %>%
  ...
  flextable::set_caption(
    "Table X. Performance of Raw and Calibrated Models"
  )
...
print(
  ...
  target = here::here(
    "tables",
    "Calibration_Results_All_Models.docx"
  )
)
...
#=========================================================
# Reload raw dataset
df_raw <- read.csv("MetSyn_dataset_Week_144.csv")
...
# Only keep those that exist in df_with_hip
cols_to_clean <- intersect(cols_to_clean, names(df_with_hip))
...
# Save the document
print(doc, target = "Supplementary_Tables_S2_S3_BMC_Style.docx")
...
cat("\n\nSUPPLEMENTARY TABLE S3\n")
cat("Partial correlation of hip circumference with MetS controlling for waist circumference\n")
cat("------------------------------------------------------------\n")
print(partial_table_s3)


# ===============================================================
# 9. REPRODUCIBILITY AND ENVIRONMENT SNAPSHOT
# ===============================================================

# Create reproducible environment lock file
renv::snapshot()

# Create human-readable session summary
writeLines(capture.output(sessionInfo()), "sessionInfo.txt")

cat("Environment snapshot and session info saved.\n")
